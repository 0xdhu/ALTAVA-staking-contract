...
describe("Secondskin NFT Staking for booster", function () {
  ...
  describe("NFTStaking: Stake should work correctly", function () {
    ...
    it("Double Stake should work", async function () {
      ...
      expect(
        await NFTStakingContract.checkStaked(owner.address, 1)
      ).to.be.equal(true);
      expect(
        await NFTStakingContract.checkStaked(owner.address, 2)
      ).to.be.equal(true);
      expect(
        await NFTStakingContract.checkStaked(owner.address, 3)
      ).to.be.equal(true);
      expect(
        await NFTStakingContract.checkStaked(owner.address, 4)
      ).to.be.equal(true);
