...
describe("MasterChef and SmartChef", function () {
  ...
  describe("NFTStaking: Deployment should work correctly", function () {
    ...
    it("SetBoosterArray should work", async function () {
      ...
      expect(await SmartChefContract.boosterTotal()).to.equal(3);
      ...
      expect(await SmartChefContract.boosterTotal()).to.equal(4);
    ...
    it("Stake should work", async function () {
      const { SmartChefContract, TAVAContract, owner } = await loadFixture(
        deployFixture
      );
    ...
    it("Extend stake should work", async function () {
      const { SmartChefContract, TAVAContract, owner, addr1, addr2 } =
        await loadFixture(deployFixture);
    ...
    it("Unlock should work", async function () {
      ...
      await expect(SmartChefContract.unlock(owner.address)).to.be.revertedWith(
        "Still in locked"
      );
    ...
    it("Admin should be able to unlock user's token", async function () {
      ...
      // ---- Alice
      // Unlock should not work before unlock days
      await expect(SmartChefContract.unlock(addr1.address)).to.be.revertedWith(
        "Still in locked"
      );
